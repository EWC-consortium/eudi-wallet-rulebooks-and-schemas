...
In the initial phase of the EUDIW era, we know that PID won't be supported by all wallets and in any case it will mandatory for interacting with PA. Probably we can foresee two parallel scenarios: the first with a wallet and official PID activated, and a second with a wallet containing an identity credential equivalent according to regulation, enriched with the information that nowadays are managed in identity proofing processes and those ones that consume that identity assertion, all onboarding processes for qualified services and others.
